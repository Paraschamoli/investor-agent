...
from investor_agent.main import handler
...
async def test_handler_returns_response():
    ...
    with patch("investor_agent.main._initialized", True), \
         patch("investor_agent.main.run_agent", new_callable=AsyncMock, return_value=mock_response):
...
async def test_handler_with_multiple_messages():
    ...
    with patch("investor_agent.main._initialized", True), \
         patch("investor_agent.main.run_agent", new_callable=AsyncMock, return_value=mock_response) as mock_run:
        result = await handler(messages)
...
async def test_handler_initialization():
    ...
    with patch("investor_agent.main._initialized", False), \
         patch("investor_agent.main.initialize_all", new_callable=AsyncMock) as _mock_init, \
         patch("investor_agent.main.run_agent", new_callable=AsyncMock, return_value=mock_response), \
         patch("investor_agent.main._init_lock"):
