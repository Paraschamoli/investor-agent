...
run-dev: ## Run the agent in development mode with auto-reload
	@echo "🤖 Starting investor-agent in development mode..."
	@uv run watchfiles "python -m investor_agent" investor_agent/
...
docker-build: ## Build Docker image
	...
	@docker build -f Dockerfile.agent -t investor_agent:latest .

.PHONY: docker-build-no-cache
docker-build-no-cache: ## Build Docker image without cache
	@echo "🐳 Building Docker image (no cache)..."
	@docker build --no-cache -f Dockerfile.agent -t investor_agent:latest .
...
docker-clean: docker-rm ## Clean up Docker images and containers
	...
	@docker rmi investor_agent:latest || true
...
docker-push: ## Push Docker image to Docker Hub
	...
	@docker tag investor_agent:latest para5/investor_agent:latest
	@docker push para5/investor_agent:latest
...
status: ## Show agent and Docker status
	...
	@docker ps -a | grep investor_agent || echo "No containers found"
	@echo "\nDocker Images:"
	@docker images | grep investor_agent || echo "No images found"
	@echo "\nAgent Health:"
